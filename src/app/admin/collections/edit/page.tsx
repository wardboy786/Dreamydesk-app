...
import { Wallpaper, CuratedCollection } from "@/lib/types";
...
export default function EditCollectionPage() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            setLoading(true);
            try {
                const [collectionData, allWallpapersData] = await Promise.all([
                ...
                const wallpapersInCollection = await getWallpapersByIds(collectionData.wallpaperIds);

                setCollection(collectionData);
                setTitle(collectionData.title);
                setDescription(collectionData.description);
                setThumbnailPreview(collectionData.thumbnailUrl);
                setIsPremium(collectionData.isPremium || false);
                setCollectionWallpapers(wallpapersInCollection);
                setAllWallpapers(allWallpapersData);

            } catch (error) {
                toast({ variant: "destructive", title: "Error", description: "Failed to load collection data." });
                notFound();
            } finally {
                setLoading(false);
            }
    ...
    const removeWallpaper = (wallpaperId: string) => {
        setCollectionWallpapers(collectionWallpapers.filter(w => w.id !== wallpaperId));
    }
    ...
    const handleSaveChanges = async () => {
        ...
        let newThumbnailUrl = collection.thumbnailUrl;
    ...
    if (loading) {
        return <EditCollectionPageSkeleton />;
    }
        ...
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold font-headline">Edit Collection</h1>
                        <p className="text-muted-foreground">
                            Modify the details and wallpapers for '{collection.title}'.
                        </p>
                    </div>
                </div>
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                ...
                                {collectionWallpapers.length === 0 && (
                                    <p className="text-center text-muted-foreground py-4">No wallpapers in this collection yet.</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
...
function EditCollectionPageSkeleton() {
    return (
